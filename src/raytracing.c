...
void		ft_ini_ray(t_rt *rt, double x, double y)
{
	...
	{	
		rt->ray->dir = ft_sub_vect(ft_add_vect(rt->view->up_left,
					ft_mult_vect(rt->view->length * x / WIN_LEN, rt->cam->left_cam->right)),
				ft_mult_vect(rt->view->height * y / WIN_HEIGHT, rt->cam->left_cam->up));
		rt->ray->dir = ft_rotation(rt->ray->dir, rt->cam->left_cam->rot);

		rt->ray->dir = ft_sub_vect(ft_add_vect(rt->view->up_left,
					ft_mult_vect(rt->view->length * x / WIN_LEN, rt->cam->right_cam->right)),
				ft_mult_vect(rt->view->height * y / WIN_HEIGHT, rt->cam->right_cam->up));
		rt->ray->dir = ft_rotation(rt->ray->dir, rt->cam->right_cam->rot);
	}
